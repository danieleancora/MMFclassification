...
attempts = 100           # number of random initializations of the classification
runfor = int(P/stepP)
...
X = np.asarray(slm[:mu,:,:], dtype=np.float32)

transmission = np.abs(np.random.randn(784,targetsize*targetsize))
X = X.reshape(X.shape[0],X.shape[1]*X.shape[2])
X = np.matmul(X,transmission)

X = np.asarray(X, dtype=np.float32)
X = X / X.max()

Y = np.asarray(labels[:mu]) 
...
plt.plot(store_test_acc_MNISTrandom.mean(axis=0))
...
X = np.asarray(slm[:mu,:,:], dtype=cp.float32)
X = zoom(X, (1, targetsize/slm.shape[1], targetsize/slm.shape[2]), order=1)
X = X.reshape(X.shape[0],X.shape[1]*X.shape[2])
X = X / X.max()
...
plt.plot(store_test_acc_MNISTzoom.mean(axis=0))
...
X = np.asarray(speckleA[:mu,:,:], dtype=np.float32)
Y = np.asarray(labels[:mu]) 
